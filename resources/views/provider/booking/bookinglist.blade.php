                        ...
                        @elseif($val->booking_status_label == 'Order Completed')
                            @if ($val->review)
                                <div class="orderdiv{{ $val->id }}">
                                    <a href="#!" class="btn btn-success view_review_btn" data-bs-toggle="modal"
                                        data-bs-target="#view-review"
                                        data-id="{{$val->id}}"
                                        data-review_id="{{ $val->review->id }}"
                                        data-booking_id="{{ $val->id }}"
                                        data-product_id="{{ $val->product_id }}"
                                        data-rating="{{ $val->review->rating }}"
                                        data-review="{{ $val->review->review }}"
                                        data-review_reply="{{ $val->review->reply ? $val->review->reply->review : '' }}">
                                        {{__('view_review')}}</a>
                                </div>
                            @endif
                        @endif
...
<div class="modal fade" id="view-review" tabindex="-1" aria-hidden="true">
	<div class="modal-dialog modal-dialog-centered">
		<div class="modal-content">
			<div class="modal-header d-flex align-items-center justify-content-between">
				<h5>{{ __('view_review') }}</h5>
			...
			<form id="addCommentsForm">
				<div class="modal-body">
					...
					<div class="mb-3">
						<label class="form-label">{{ __('Ratings') }}</label>
						<div class="review_rating_list mb-0">
						</div>
					</div>
					<div class="mb-0">
						<label class="form-label">{{ __('Review') }}</label>
						<textarea class="form-control" name="review" id="review" rows="3" maxlength="500"></textarea>
						<span class="error-text text-danger" id="review_error"></span>
					</div>
                    <div class="mb-0">
						<label class="form-label">{{ __('Reply') }}</label>
						<textarea class="form-control" name="reply_review" id="reply_review" rows="3" maxlength="500"></textarea>
						<span class="error-text text-danger" id="reply_review_error"></span>
					</div>
				</div>
				<div class="modal-footer d-flex align-items-center justify-content-end">
					<a href="javascript:void(0);" class="btn btn-light me-2" data-bs-dismiss="modal">{{ __('Cancel') }}</a>
					<button type="submit" class="btn btn-primary" id="save_comment_btn">{{ __('Reply') }}</button>
				</div>
			</form>
		</div>
	</div>
				...
				<div class="mb-3 review-section d-none">
					...
					<div class="d-flex flex-column gap-2">
						...
						<label class="d-flex gap-2">
							...
							<span class="text-dark flex-grow-1" id="provider_reply"></span>
						</label>
