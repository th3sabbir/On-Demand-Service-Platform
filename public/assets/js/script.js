...
$(document).ready(function(){
	...
	var disableMiniSidebarHover = false;
	feather.replace();
	...
	$(document).on('mouseover', function(e) {
        e.stopPropagation();
        if (disableMiniSidebarHover) {
            return false;
        }
        if ($('body').hasClass('mini-sidebar') && $('#toggle_btn').is(':visible')) {
            if ($(e.target).closest('#toggle_btn').length) {
                return false;
            }
	...
	$(document).on('click', '#toggle_btn', function(e) {
		...
		e.stopPropagation();
		var $body = $('body');
		disableMiniSidebarHover = true;
		setTimeout(function() {
			disableMiniSidebarHover = false;
		}, 150);
		if ($body.hasClass('mini-sidebar')) {
			$body.removeClass('mini-sidebar expand-menu');
			$(this).addClass('active');
			$('.subdrop + ul').slideUp(0);
